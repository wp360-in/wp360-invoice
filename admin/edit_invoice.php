...
if (isset($_SERVER["REQUEST_METHOD"]) && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_invoice']) && isset($_POST['_wpnonce_add_invoice']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wpnonce_add_invoice'])), 'add-invoice-nonce-action')) {
    ...
    if (isset($_POST['_wpnonce_add_invoice']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wpnonce_add_invoice'])), 'add-invoice-nonce-action')) {
        if (!empty($invoiceTitle) && !empty($invoiceAmount) && !empty($invoiceUser)) {
            $invoice_id = wp_update_post(array(
                ...
                'meta_input'  => [
                    ...
                    'invoice_currency' => $invoiceCurrency,  // ← saved per invoice
        ...
        if ($invoice_id) {
            echo '<div class="updated"><p>' . esc_html__('Invoice updated successfully!', 'wp360-invoice') . '</p></div>';
        } else {
            echo '<div class="error"><p>' . esc_html__('Error updating the invoice.', 'wp360-invoice') . '</p></div>';
        }
...
$invoiceCurrency = get_post_meta($invoiceID, 'invoice_currency', true);
if (empty($invoiceCurrency)) {
    $invoiceCurrency = get_option('wp360_selected_currency', 'USD');
}

// Build currency list from saved textarea
$currency_list_raw = get_option('wp360_currency_list', "INR\nUSD\nEUR\nGBP\nAUD\nCAD\nJPY\nCHF\nCNY\nSGD");
